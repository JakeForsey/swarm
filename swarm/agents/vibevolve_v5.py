...
def act(
    ...
    REGEN_HEALTH_THRESHOLD = 0.1
    DAMAGE_THRESHOLD = 0.5  # Only attack if enemy is damaged

    # Compute distances to enemies for each ally
    ally_to_enemy_dx = enemy_x[:, None, :] - ally_x[:, :, None]
    ally_to_enemy_dy = enemy_y[:, None, :] - ally_y[:, :, None]
    ...
    # Determine enemy health for attack prioritization
    enemy_health_min = jnp.min(enemy_health, axis=1, keepdims=True)
    is_enemy_weak = (enemy_health < (enemy_health_min + 0.2)) & (enemy_health < DAMAGE_THRESHOLD)
    ...
    regen_health = ally_health < REGEN_HEALTH_THRESHOLD
    regen_health_factor = jnp.where(regen_health, 1.0, 0.0)
    ...
    ally_mask = jnp.any(ally_dist_sq < (AVOID_RADIUS ** 2), axis=2)
    ...
    enemy_mask = jnp.any(enemy_dist_sq < (AVOID_RADIUS ** 2), axis=2)

    # Compute group center based on ally health
    group_health_weight = jnp.clip(ally_health, 0, 1) * GROUP_HEALTH_WEIGHT
    group_center_x = jnp.sum(ally_x * group_health_weight, axis=1, keepdims=True) / jnp.sum(group_health_weight, axis=1, keepdims=True)
    group_center_y = jnp.sum(ally_y * group_health_weight, axis=1, keepdims=True) / jnp.sum(group_health_weight, axis=1, keepdims=True)
    ...
    separation_factor = jnp.where(ally_dist_sq < (AVOID_RADIUS ** 2), 1.0 / (ally_dist_sq + 1e-8), 0.0)
    ...
    # Attack behavior with prioritization of weak enemies
    attack_x = direction_x * is_attacking * ATTACK_WEIGHT
    attack_y = direction_y * is_attacking * ATTACK_WEIGHT
    attack_x = jnp.where(is_enemy_weak, attack_x * 1.5, attack_x)  # Prioritize weak enemies
    steering_x += attack_x
    steering_y += attack_y
    ...
    # Apply avoid factor and health factor
    avoid_factor = jnp.where(ally_mask, 0.1, 1.0)
    avoid_factor = jnp.where(enemy_mask, 0.1, avoid_factor)
    avoid_factor = jnp.where(is_avoiding, 0.1, avoid_factor)
    ...
    return movement_dir_x, movement_dir_y
